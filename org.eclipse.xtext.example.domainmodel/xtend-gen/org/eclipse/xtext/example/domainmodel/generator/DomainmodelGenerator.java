...
public class DomainmodelGenerator implements IGenerator2 {
  ...
  public void doGenerate(final ResourceSet rs, final IFileSystemAccess fsa) {
    {
      ...
      final Function1<Resource,Iterable<Entity>> _function = new Function1<Resource,Iterable<Entity>>() {
          public Iterable<Entity> apply(final Resource r) {
            Iterable<EObject> _allContentsIterable = ResourceExtensions.allContentsIterable(r);
            Iterable<Entity> _filter = IterableExtensions.<Entity>filter(_allContentsIterable, org.eclipse.xtext.example.domainmodel.domainmodel.Entity.class);
            return _filter;
          }
        };
      List<Iterable<Entity>> _map = ListExtensions.<Resource, Iterable<Entity>>map(_resources, _function);
      Iterable<Entity> _flatten = IterableExtensions.<Entity>flatten(_map);
      final Iterable<Entity> entities = _flatten;
      StringConcatenation _compile = this.compile(entities);
      fsa.generateFile("entities.txt", _compile);
    }
  }
  
  public StringConcatenation compile(final Iterable<Entity> entities) {
